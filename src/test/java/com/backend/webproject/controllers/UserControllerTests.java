 ...
 * org.springframework.boot.test.mock.mockito.MockBean; import static
 * org.mockito.BDDMockito.given; import org.springframework.http.MediaType;
 ...
 * 
 * import com.backend.webproject.entity.User; import
 * com.backend.webproject.managers.UserManager; import com.google.gson.Gson;
 * 
 * import static
 ...
 * @SpringBootTest
 * 
 * @AutoConfigureMockMvc class UserControllerTests {
 * 
 * @Autowired private MockMvc mockMvc;
 * 
 * @MockBean private UserManager userManager;
 * 
 * @Test public void controllerRoutingTest() throws Exception {
 ...
 * 
 * @Test public void loginUserAPITest() throws Exception { User user = new
 * User(1, "al@gmail.com", "pass123", "Alberto", "8110", 1);
 * 
 ...
 * 
 * // String response = mockMvc.perform( post("/user/api/login") .content(json)
 ...
 * .andExpect(jsonPath("id", is(user.getId()))) .andExpect(jsonPath("role",
 * is(user.getRole()))); // .andReturn().getResponse().getContentAsString();
 * 
 * // UserEntity userData = gson.fromJson(response, UserEntity.class); //
 * System.out.println("response string: " + response); //
 * System.out.println("response object: " + userData); }
 * 
 * @Test public void signupUserAPITest() throws Exception { User user = new
 * User(1, "al@gmail.com", "pass123", "Alberto", "8110", 1);
 * 
 * given(userManager.signupUser("al@gmail.com", "pass123", "Alberto",
 * "8110")).willReturn(user);
 * 
 * Gson gson = new Gson(); String json = gson.toJson(user);
 * 
 * System.out.println("json string: " + json);
 * 
 * // String response = mockMvc.perform( post("/user/api/signup") .content(json)
 * .accept(MediaType.APPLICATION_JSON) .contentType(MediaType.APPLICATION_JSON))
 * .andDo(print()) .andExpect(jsonPath("email", is(user.getEmail())))
 * .andExpect(jsonPath("password", is(user.getPassword())))
 * .andExpect(jsonPath("fullname", is(user.getFullname())))
